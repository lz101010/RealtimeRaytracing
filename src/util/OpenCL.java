...
public class OpenCL {
    ...
    public static CLMem clCreateFromGLTexture2D(CLContext context, int flags, int target, int mimap, int texture) {
        CLMem mem = CL10GL.clCreateFromGLTexture2D(context, flags, target, mimap, texture, lastErrorCode);
        checkError();
        return mem;
    }
    
    public static void clEnqueueReadBuffer(CLCommandQueue queue, CLMem src, int blocking_read, long offset, FloatBuffer buffer, PointerBuffer event_wait_list, PointerBuffer event) {
        lastErrorCode.put(0, CL10.clEnqueueReadBuffer(queue, src, blocking_read, offset, buffer, event_wait_list, event));
        checkError();
    }
    
    
    public static void clEnqueueReadBuffer(CLCommandQueue queue, CLMem dst, int blocking_read, long offset, ByteBuffer buffer, PointerBuffer event_wait_list, PointerBuffer event) {
        lastErrorCode.put(0, CL10.clEnqueueReadBuffer(queue, dst, blocking_read, offset, buffer, event_wait_list, event));
        checkError();
    }
    ...
    public static void clEnqueueWriteBuffer(CLCommandQueue command_queue, CLMem mem_object, int blocking_writing, long offset, Buffer ptr, PointerBuffer event_wait_list, PointerBuffer event) 
    {
        ...
        {
    		lastErrorCode.put(0, CL10.clEnqueueWriteBuffer(command_queue, mem_object, blocking_writing, offset, (ByteBuffer)ptr, event_wait_list, event));   
        }
        else if(ptr instanceof FloatBuffer)
        {
        	lastErrorCode.put(0, CL10.clEnqueueWriteBuffer(command_queue, mem_object, blocking_writing, offset, (FloatBuffer)ptr, event_wait_list, event));   
        }
        else if(ptr instanceof IntBuffer)
        {
        	lastErrorCode.put(0, CL10.clEnqueueWriteBuffer(command_queue, mem_object, blocking_writing, offset, (IntBuffer)ptr, event_wait_list, event));
        }
        else
        {
    ...
    /**
     * Gibt au�erdem compiler Fehler aus.
